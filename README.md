...
3. Highlight text on a regular web page (Chrome system pages such as `chrome://` or the Web Store block extensions). A polished, light/dark-aware Knowledge Dump window slides in with a **Snippet/Page** toggle. Snippet mode shows the captured **Content** plus required **Tag**/**Title** fields, and optional Author (the page URL is captured automatically in the background). Page mode focuses purely on saving the current page (fields shown in the order **Title → Tag → Author**); the URL is auto-filled and sent silently, so you don’t have to see or edit it.
